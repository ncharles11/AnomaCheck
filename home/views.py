...
class CustomPasswordResetView(PasswordResetView):
    template_name = 'auth/forgot_password.html'  # Formulaire de demande de réinitialisation
    email_template_name = 'emails/password_reset_email.html'  # Email envoyé à l'utilisateur
    subject_template_name = 'emails/password_reset_subject.txt'  # Sujet de l'email
    success_url = reverse_lazy('forgot_password_done')  # Redirection après succès

class CustomPasswordResetConfirmView(PasswordResetConfirmView):
    template_name = 'auth/password_reset_confirm.html'  # Template pour la réinitialisation
...
def next_page(request):
    return render(request, 'extra/next_page.html')
...
def patch_selection(request):
    """Affiche la page de sélection du patch après vérification de l’image"""
    uploaded_image = request.session.get("uploaded_image")

    if not uploaded_image:
        messages.error(request, "Veuillez téléverser une image avant de choisir un patch.")
        return redirect("upload_page")  # ✅ Sécurisation si l'image n'est pas en mémoire

    return render(request, "extra/patch_selection.html", {"uploaded_image": uploaded_image})


def process_patch_selection(request):
    """Stocke le choix du patch et redirige vers la page de confirmation"""
    if request.method == "POST":
        patch_size = request.POST.get("patch_size")

        if not patch_size:
            return render(request, "extra/patch_selection.html", {"error": "Veuillez choisir une taille de patch."})

        # Stocke le choix en session
        request.session["patch_size"] = patch_size

        # Redirige vers une page de confirmation (ou directement vers la prédiction plus tard)
        return redirect("confirm_patch")

    return redirect("patch_selection")

def confirm_patch(request):
    """Affiche un résumé avant de passer à l’analyse"""
    patch_size = request.session.get("patch_size")
    uploaded_image = request.session.get("uploaded_image")

    if not patch_size or not uploaded_image:
        return redirect("upload_page")  # Sécurité : si aucune image n’est enregistrée, on revient au début

    return render(request, "extra/confirm_patch.html", {"patch_size": patch_size, "image_url": uploaded_image})
